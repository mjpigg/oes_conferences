...
cur.execute("CREATE TABLE pcr (grade,advisor,lastname,firstname,dept,course,teacherlast,parentemail1,parentemail2,studentID INT, period,parent1first,parent1last,parent2first,parent2last);") # use your column names here
...
cur.executemany("INSERT INTO pcr (grade,advisor,lastname,firstname,dept,course,teacherlast,parentemail1,parentemail2,studentID, period,parent1first,parent1last,parent2first,parent2last) VALUES (?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?);", to_db)
...
cur.executemany("INSERT INTO confs (studentID,grade,firstname,lastname,advisor) VALUES (?,?,?,?,?);",to_db)


#Second pass, UPDATE confs to add English, Hum_Hist,Language,Math,PE, Science
sql="SELECT studentID,dept,course,teacherlast,period FROM pcr WHERE dept NOT LIKE 'Perform%' AND dept NOT LIKE 'Acti%' GROUP BY studentID,course;"
cur.execute(sql)
rows=cur.fetchall()

col_map={'Languages':'language',
'Science':'science',
...
sql="SELECT studentID, teacherlast from pcr where dept LIKE 'perf%' AND (course LIKE 'Act%' OR course like 'Art%') GROUP BY course,studentID;"
cur.execute(sql)
rows=cur.fetchall()
for i in rows:
...
#fourth, UPDATE Music
sql="SELECT studentID, teacherlast from pcr where dept LIKE 'perf%' AND (course LIKE 'Band%' OR course like 'Choir%') GROUP BY course,studentID;"
cur.execute(sql)
rows=cur.fetchall()
for i in rows:
